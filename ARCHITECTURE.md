...
- **Who can read what is written down and cannot rot.** `docs/rls-matrix.md` is
  ...
  regenerated by `npm run rls:matrix` and asserted current by
  `tests/db/rls-matrix.test.ts`. Reads are **executed** against the real
  policies; writes are **read from `pg_policy`**, because executing an INSERT
  on 36 tables means inventing valid rows for all of them and the catalog
  answers the question completely anyway: only six tables grant a browser any
  write, and no table has a DELETE policy. A third answer, **`no data`**, is
  printed wherever a table held no rows to demonstrate with — the same rule as
  a column default never being presented as a measurement.
...
| Live routes | `npm run test`, `npm run check:deployed` | That every page in the repository is on a walked list, and that a guarded route answers a redirect to its own language's login rather than 200 — proven by dropping `/admin/signals` from the list and watching the coverage pass go red, and by removing the locale prefix from `middleware.ts` and watching the agreement test go red, and by accepting a 404 from a cron target and watching both the test and the script's own self-test go red |
