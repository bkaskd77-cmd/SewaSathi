...
--      EXECUTE to PUBLIC on every new function. Calling a trigger function
--      directly errors out, so this was never much of a door, but a function
--      nobody is supposed to call by hand should not be callable by hand.
--
...
-- 3. Take away the default grant to PUBLIC.
--
...
--    and fails if either stops working.
revoke execute on function public.enforce_booking_transition() from public;
revoke execute on function public.enforce_payment_transition() from public;
revoke execute on function public.record_booking_status() from public;
...
revoke execute on function public.booking_transition_allowed(text, text) from public;
revoke execute on function public.payment_transition_allowed(text, text) from public;

--    is_admin(): PUBLIC loses it, `authenticated` keeps it, for the reason at
--    the top of this file. Granted explicitly so the intent is in the schema
