...
- **Still missing and deliberately not covered by this entry: there is no admin screen to issue a guarantee refund.** The db suite proves one needs an admin (`money back needs a person`), and today that means database access. It is a new money write path and gets its own phase; a refund is the rare capped case the policy already describes, not the guarantee's normal operation, which is a re-do.
